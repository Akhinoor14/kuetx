...
**Status: ✅ Done — item 3 needed real code (new `RequireGuestMode`
guard); items 1, 2, 4, 5 were confirmed already-correct by construction
and needed no code changes. `npx vite build` passes clean.**

### Findings / Context (for future phases)

- **Item 1 (direct link/bookmark) — already worked, verified not
  built.** `/guest/dashboard` etc. are plain top-level `<Route>` entries
  in `PUBLIC_PATHS` (Phase 1/2's mechanism) — `buildQueue()` returns an
  empty queue for any `PUBLIC_PATHS` path regardless of how the visitor
  arrived there, so a bookmarked/typed `/guest/schedule` URL renders
  correctly with no dependency on having clicked through `/about` first.
  No code needed.
- **Item 2 (back button) — already worked, verified not built.** Since
  `/about` and every `/guest/*` route render through the normal
  `<Routes>` tree (no overlay, no modal state involved once the queue is
  empty — see Phase 1's Findings on how the gate fix works), browser
  back/forward between them is just normal React Router history
  navigation. No auth-wall flash exists to fix because the auth-wall
  overlay is never mounted for these paths in the first place.
- **Item 3 (signed-in user hitting `/guest/...`) — real gap, now
  fixed.** Confirmed by reading `App.jsx`'s route list before this phase:
  the four `/guest/*` routes had zero auth check — a signed-in account
  landing on `/guest/dashboard` (e.g. an old bookmark from before they
  had an account) would see the fake demo data with no indication it
  wasn't their own. New file `src/components/RequireGuestMode.jsx`
  mirrors the existing `RequireStudentMode.jsx`/`RequireFaculty.jsx`
  wrapper-guard pattern already used elsewhere in `App.jsx`'s route list,
  rather than inventing a new mechanism. Takes `authState` as a prop
  (the same object `Layout` already receives and the four guest
  `<Route>` elements render inside, so `authState` is in scope with no
  extra plumbing) — redirects to `/dashboard` when
  `authState.authReady && authState.user` are both truthy, otherwise
  passes through untouched. Deliberately does not attempt per-role
  redirect logic itself (e.g. sending faculty straight to
  `/faculty`) — `/dashboard`'s own existing role resolution
  (`RootRouteResolver` et al.) already handles that correctly once the
  person lands there, so duplicating it here would be redundant.
  All four `/guest/dashboard|schedule|attendance|marks` routes in
  `App.jsx` now wrap their element in `<RequireGuestMode
  authState={authState}>`.
- **Item 4 (mobile responsiveness) — already correct, verified not
  built.** Read `GuestBanner.jsx` and `GuestNav.jsx` directly: both use
  `flexWrap: 'wrap'` with no fixed pixel widths, `NavLink`s and the
  banner's text/button both wrap naturally at narrow viewports. No
  narrow-viewport-specific bug found; no change made.
- **Item 5 (`usePageMeta` on About) — already correct, verified not
  built.** Read `src/hooks/usePageMeta.js` directly: it's a plain
  `useEffect` DOM mutation (`document.title` / meta description) with no
  dependency on auth state, Firestore, or any store read — it fires
  identically whether `About.jsx` renders for a signed-in or signed-out
  visitor. No change made.
- **Files touched this phase:** `src/components/RequireGuestMode.jsx`
  (new), `src/App.jsx` (one new import, four `<Route>` elements wrapped).
  No changes to `GuestBanner.jsx`, `GuestNav.jsx`, `usePageMeta.js`, or
  any other Phase 1–3 file.
...
**Status: ✅ Done — role-based resolution implemented exactly as
5.1 specifies (resolvedRole prop, computed by each caller from the real
useIsFaculty/useIsProvider/useIsStaff hooks, not from useLocation()).
All four new Overview categories/sections added per 5.2. 5.4's guide
entry point question resolved by choosing the plan's offered alternative
— see Findings. `npx vite build` passes clean.**

### Findings / Context (for future phases)

- **5.1 — `GuideModal.jsx` changes:** `getShellContext(pathname)` deleted
  entirely (grep confirms zero remaining references except this file's
  own explanatory comment). New `GuideModal({ open, onClose, isViewerCR
  = false, resolvedRole })` signature — `shell` is now `resolvedRole ||
  DEFAULT_ROLE` (`DEFAULT_ROLE = 'student'`, used only as a safety
  fallback if some future caller forgets to pass `resolvedRole`; every
  real call site as of this phase does pass it). `useLocation()` import
  removed from `GuideModal.jsx` entirely — nothing in the file reads
  `location` anymore, `useNavigate()` is the only react-router-dom hook
  still needed (for the "Open page" button's `navigate(active.route)`).
- **5.1 — six resolved values, `visibleCategories` map:**
  ```js
  {
    guest:        [8],           // Guest Overview only
    student:      [0, 1, 2, 3],  // unchanged from before this phase
    'student-cr': [0, 1, 2, 3],  // same base list; CR category (4) is
                                  // appended separately, see below
    faculty:      [9, 5],        // Faculty Overview + Faculty Portal
    provider:     [10, 6],       // Provider Overview + Service Provider
    staff:        [11, 7],       // Staff Overview + Team & Admin
  }
  ```
  CR-merge kept working exactly as before: `isStudentShell = shell ===
  'student' || shell === 'student-cr'`, and category 4 (CR) is appended
  only when `isStudentShell && isViewerCR` — same trigger condition as
  the pre-Phase-5 code, just no longer hardcoded to the literal string
  `'student'`.
- **5.1 — three call sites updated, one each:**
  - `Navbar.jsx`: added `useIsStaff` import (mirroring the file's
    existing `useIsProvider`/`useIsFaculty` imports). Computes
    `resolvedRole` inline at the `<GuideModal>` call with priority
    faculty → provider → staff → student-cr → student. `useIsStaff()`
    returns `{ isRealAdmin, adminLabel, isResolved }` (confirmed by
    reading the hook directly — no `isStaff` field, unlike
    `useIsFaculty`/`useIsProvider`'s naming) — destructured as `const {
    isRealAdmin: isStaff } = useIsStaff();` to keep the call-site ternary
    readable. Navbar only ever renders for a signed-in user (guest mode
    uses `GuestShell`, a completely separate component tree per Phase
    2.1's routing split), so `'guest'` is never produced here — that
    value only ever comes from the other two call sites, which can
    genuinely be signed-out or not-yet-role-selected.
  - `AuthModal.jsx`: hardcoded `resolvedRole="guest"` — no hook needed,
    since by definition anyone seeing this modal hasn't completed sign-in
    yet.
  - `RoleSelectScreen.jsx`: also hardcoded `resolvedRole="guest"` — this
    screen's whole purpose is that the signed-in account hasn't picked
    Student/Faculty/Provider yet, so no faculty/provider/staff role
    exists yet to resolve to. Guessing which role they're about to pick
    would be wrong more often than right; 'guest' Overview ("what is
    KUETx, what do the roles do") is the honest fit for someone still
    deciding.
- **5.2 — four new categories, four new sections.** Added at
  `GUIDE_CATEGORIES_BN`/`_EN` indices 8-11 (`গেস্ট — KUETx কী?` /
  "Guest Overview", `ফ্যাকাল্টি — শুরুর কথা` / "Faculty Overview",
  `প্রোভাইডার — শুরুর কথা` / "Provider Overview", `স্টাফ — শুরুর কথা` /
  "Staff Overview") — kept fully distinct from index 0's student
  "শুরুর কথা"/"Overview", per this section's explicit instruction. Four
  new `RAW_SECTIONS` entries (`num: "60"`-`"63"`, ids `guest-overview`,
  `faculty-overview`, `provider-overview`, `staff-overview`) added at the
  end of the array, following the exact same shape as every existing
  entry (`titleBn/titleEn/descBn/descEn/blocksBn/blocksEn`, `catIdx`
  pointing at the new `CAT.*_OVERVIEW` constants). Each kept to
  2 `text`/`bullet`-style blocks plus one closing `callout` — 4-6 blocks
  total, matching the section's "2-4 short sections, not a dozen"
  guidance loosely by block-count rather than a strict section-count,
  since these are single sections with a few blocks each, not multiple
  sub-sections.
  - **Faculty Overview's verification framing** pulled directly from
    `RequireVerifiedFaculty.jsx`'s doc comment (read before writing this
    section, per 5.2's own instruction): notices + marks entry need
    Founder-approved Teacher Verification, everything else (classes,
    sessions, schedule) works right after signup — worded to match that
    file's actual policy language, not invented separately.
  - **Staff Overview** explicitly notes CR is NOT in the staff catalog
    (pulled from `staffRoles.js`'s own top-of-file comment: "CR is
    deliberately NOT in this catalog... a per-class student feature, not
    an official KUETx post") — included as a callout so the distinction
    is explicit for a reader who might otherwise conflate the two.
  - **Icon names corrected during implementation:** the section
    originally called for `Compass` (guest) and `ShieldCheck` (staff),
    but neither is registered in `iconRegistry.js` (confirmed by
    grepping the file directly) — `Icon`'s fallback to a bare `Circle`
    would have rendered silently-wrong icons rather than erroring, so
    this would NOT have been caught by the build. Swapped to `Sparkles`
    (guest — already used elsewhere in this file for overview-type
    sections) and `Shield` (staff — the registered variant without
    "Check"). `GraduationCap` and `Store` (faculty/provider) were already
    registered, no change needed there.
- **5.3 — `visibleCategories` composition:** implemented exactly as
  5.2/5.1's Findings above describe; no separate work beyond what's
  already documented in 5.1's map.
- **5.4 — guest guide entry point: chose the "fold into `/about`'s body
  copy" alternative, not a guest-nav `?` icon.** Verified guest mode
  (`GuestShell.jsx`, `GuestBanner.jsx`, `GuestNav.jsx`,
  `src/pages/guest/*.jsx`) has zero `GuideModal`/`guideOpen` references
  today (grep confirms) — there was never a guide icon in guest mode to
  begin with, so this was a genuine choice between two paths, not a
  removal. `About.jsx`'s existing hero copy ("Campus life, connected."
  section, confirmed present before this phase) already explains what
  KUETx is to every visitor, signed in or not, and Phase 1 already added
  the Guest Overview content's core idea in guide form.  Adding a guide
  icon to guest pages as well would duplicate that same "what is KUETx"
  explanation in two places for the same audience — the section's own
  guidance was "pick whichever avoids duplicating the same explanatory
  content in two places," so no guide icon was added to
  `GuestShell.jsx`/`GuestBanner.jsx`/`GuestNav.jsx`. The `guest-overview`
  guide section built in 5.2 is still real, reachable content — it
  serves the `AuthModal.jsx`/`RoleSelectScreen.jsx` call sites (signed-
  out visitors mid-signup, and signed-in accounts pre-role-selection),
  it's simply not ALSO duplicated into the separate guest-preview route
  tree.
- **Files touched this phase:** `src/data/guideContent.js` (categories +
  4 new sections), `src/components/GuideModal.jsx` (role-based
  resolution, `useLocation` removed), `src/components/Navbar.jsx`
  (`useIsStaff` import + `resolvedRole` computation),
  `src/components/AuthModal.jsx` (`resolvedRole="guest"`),
  `src/components/RoleSelectScreen.jsx` (`resolvedRole="guest"`). No
  other files changed — `guideContent.js`'s existing 60+ sections for
  the original four roles are untouched (verified: only the closing `]`
  of `RAW_SECTIONS` was edited to insert new entries before it, nothing
  in the existing entries was touched).
