...
1. Fix the straightforward display-only consumers (Courses.jsx, TermQS.jsx,
   Assignments.jsx, TermPlanner.jsx) — low risk, same resolver pattern
   used several times already this phase.
2. Check CTQuizPlanning.jsx's actual data flow (local vs mirrored) before
   deciding if it needs a fix.
3. Fix the facultyDisambiguation.js stale comment (trivial, do whenever).
4. Manual test pass in the real dev/Vercel environment — nothing in this
   phase has been tested outside of the isolated `teacherRegistry.js`
   logic tests and esbuild syntax checks. Attendance.jsx specifically
   deserves a real manual pass (mark present/absent, switch teacher,
   rotation pick) even though no code changed there, since it's
   downstream of everything else that DID change (Schedule.jsx's
   getCourseTeachers, the 3 CourseTeacherDialog write sites).
