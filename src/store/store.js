...
// ---------------- Schedule Period System ----------------
// Period = academic concept (1-9 regular, L1-L3 lab). Time = display only,
// derived from the active model. Switching model = zero data migration.
// Lab boundary: L1 = P1-P3, L2 = P4-P6, L3 = P7-P9.

export const ALL_PERIODS = [1, 2, 3, 4, 5, 6, 7, 8, 9];
export const ALL_LAB_PERIODS = ['L1', 'L2', 'L3'];

export const PERIOD_MODELS = {
  '50min': {
    id: '50min',
    name: '50-min Periods',
    note: '8:00 AM start · 50-min classes',
    breakAfter: 6,
    periods: {
      1: '8:00 AM - 8:50 AM',
      2: '8:50 AM - 9:40 AM',
  ...
  '40min': {
    id: '40min',
    name: '40-min Periods',
    note: '9:00 AM start · 40-min classes',
    breakAfter: 6,
    periods: {
      1: '9:00 AM - 9:40 AM',
      2: '9:40 AM - 10:20 AM',
...
// period (1-9 number or 'L1'/'L2'/'L3') + modelId -> time string for display.
// Returns null if period/model is invalid (caller falls back to legacy slot text).
export const periodToTime = (period, modelId) => {
  const model = PERIOD_MODELS[modelId] || PERIOD_MODELS['50min'];
  if (period === null || period === undefined || period === '') return null;
  const key = String(period);
  if (key.startsWith('L')) return model.labs[key] || null;
  const time = model.periods[Number(key)];
  return time || null;
};

// Normalize a legacy slot string for lookup: collapse whitespace, normalize dash/arrow.
const normalizeSlotKey = (slot) => String(slot || '')
  .replace(/\u2192/g, '-') // → to -
  .replace(/\s*-\s*/g, ' - ')
  .replace(/\s+/g, ' ')
  .trim();

// Reverse lookup table built once from PERIOD_MODELS (covers both models,
// so a slot saved under either model's time strings still resolves).
const SLOT_TO_PERIOD_MAP = (() => {
  const map = {};
  Object.values(PERIOD_MODELS).forEach(model => {
    Object.entries(model.periods).forEach(([p, time]) => {
      map[normalizeSlotKey(time)] = Number(p);
    });
    Object.entries(model.labs).forEach(([l, time]) => {
      map[normalizeSlotKey(time)] = l;
    });
  });
  return map;
})();

// Legacy time string -> period (number or 'L1'/'L2'/'L3'). Returns null if unrecognized.
export const timeToPeriod = (slot, _modelId) => {
  if (!slot) return null;
  const key = normalizeSlotKey(slot);
  if (key in SLOT_TO_PERIOD_MAP) return SLOT_TO_PERIOD_MAP[key];
  return null;
};

// One-time migration: convert legacy time-string `slot` entries in the schedule
// to `period`-based entries. Idempotent — safe to call on every app boot.
// Never deletes data: unrecognized slots are kept as-is (period becomes 'unknown',
// legacy slot text preserved as _legacySlot) instead of being dropped.
export const migrateScheduleToPeriods = () => {
  try {
    const schedule = store.get('schedule');
    if (!Array.isArray(schedule) || schedule.length === 0) return;

    // Already migrated if every entry already has a period.
    const needsMigration = schedule.some(entry => entry && entry.period === undefined);
    if (!needsMigration) return;

    const settings = store.get('scheduleSettings') || {};
    const modelId = PERIOD_MODELS[settings.modelId] ? settings.modelId : '50min';

    let changed = false;
    const migrated = schedule.map(entry => {
      if (!entry || entry.period !== undefined) return entry;
      const period = timeToPeriod(entry.slot, modelId);
      changed = true;
      if (period === null || period === undefined) {
        // Unknown slot format: keep legacy slot text untouched, flag for visibility.
        return { ...entry, period: 'unknown', _legacySlot: entry.slot };
      }
      return { ...entry, period, _legacySlot: entry.slot };
    });

    if (changed) {
      store.set('schedule', migrated);
      recordAudit({ action: 'migrate_schedule_to_periods', count: migrated.length });
    }
  } catch (e) {
    console.error('[KUETx Store] Schedule period migration error:', e);
  }
};