...
function AvatarUploadModal({ currentURL, isAnon, onClose, onUploaded, onDeleted }) {
  ...
  const handleFile = (f) => {
    ...
    if (f.size > 3 * 1024 * 1024) { setError('Max 3 MB'); return; }
    setError('');
    setFile(f);
  ...
  const handleUpload = async () => {
    ...
    try {
      const url = await uploadProfilePicture(file, setProgress);
      onUploaded(url);
      onClose();
    } catch (e) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {isAnon && (
          <div style={{ padding: '10px 14px', borderRadius: 10, background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)', fontSize: 12, color: '#b45309' }}>
            ⚠️ Sign in করলে ছবি সব device-এ sync হবে। Guest mode-এ শুধু এই device-এ থাকবে।
          </div>
        )}
        ...
        <div
          ...
          style={{
            ...
            padding: 24, border: '2px dashed var(--border)', borderRadius: 16,
        ...
        >
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>JPG, PNG, WebP · Max 3 MB · Auto-resized to 400×400</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          {currentURL && (
            <button onClick={handleDelete} style={{
              padding: '10px 16px', borderRadius: 9, border: '1px solid rgba(239,68,68,0.3)',
              background: 'rgba(239,68,68,0.07)', color: '#ef4444', fontSize: 13, fontWeight: 600, cursor: 'pointer',
            }}>Remove</button>
          )}
          <button
