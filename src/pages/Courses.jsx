...
function CustomCourseForm({ initial, onSave, onCancel }) {
  ...
  return (
    <div className="card mb-3" style={{ borderColor: 'var(--accent)', borderWidth: 2 }}>
      <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 12 }}>{initial?.id ? 'Edit Custom Course' : '+ Add Custom Course'}</div>
...
export default function Courses() {
  ...
  const viewCourseSyllabus = (id) => { store.set('selectedSyllabusCourseid', id); navigate('/syllabus'); };
  const updateOverride = (id, patch) => { setCourseOverride(id, patch); setVersion(v => v + 1); };
  const updateOptional = (course, code) => { setOptionalSelection({ deptCode: course.deptCode, termKey: Y + course.year + T + course.term, slotIndex: course.optionalSlotIndex, code }); setVersion(v => v + 1); };
                ...
                <div key={c.id} className="card" style={{ display:'flex', alignItems:'flex-start', gap:12, padding:'10px' }}>
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginTop:8, alignItems:'center' }}>
                      <span className={	ag  + STATUS_COLORS[c.status]||'tag-gray'} style={{ fontSize:12, padding:'4px 8px' }}>{c.status}</span>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:6, minWidth:110, width:110 }}>
                    {c.isOptional && (
                      <select value={c.optionalCode||''} onChange={e=>updateOptional(c,e.target.value)} style={{ fontSize:13, padding:6 }}>
                        <option value="">Select</option>
                        {optionalCatalog.map(opt => <option key={opt.code} value={opt.code}>{opt.code} — {opt.title}</option>)}
                      </select>
                    )}
                    <select value={c.status} onChange={e=>updateOverride(c.id, { status: e.target.value })} style={{ fontSize:13, padding:6 }}>
        ...
        <div style={{ marginTop:24 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            {customCourses.map(c => (
              <div key={c.id}>
                {editingCustom === c.id ? (
                  <CustomCourseForm initial={c} onSave={(f)=>{ setCustomCourses(customCourses.map(x=>x.id===f.id?f:x)); setVersion(v=>v+1); setEditingCustom(null); }} onCancel={()=>setEditingCustom(null)} />
                ) : (
                  <div className="card" style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 16px' }}>
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginTop:6 }}>
                        <span className={	ag  + STATUS_COLORS[c.status]||'tag-gray'}>{c.status}</span>
