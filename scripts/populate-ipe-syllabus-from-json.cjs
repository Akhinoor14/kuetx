...
const jsonPath = path.join(__dirname, '..', 'sylla', 'iemcuriculmn.json');
const outDir = path.join(__dirname, '..', 'src', 'data', 'curriculum', 'departments', 'IPE', 'syllabus');
...
const buildCourseObject = course => ({
  ...
  sessionalNote: course.sessionalNote ?? (String(course.title || '').toLowerCase().includes('sessional') ? course.title : null),
...
for (const termKey of termOrder) {
  ...
  const exportName = `IPE_SYLLABUS_${termKey}`;
  termImports.push(`import { ${exportName} } from './${termKey}.js';`);
  ...
  for (const [code, course] of Object.entries(term.courses || {})) {
    termObj.courses[code] = buildCourseObject(course);
  }
...
const optionalContent = `export const IPE_SYLLABUS_OPTIONAL = ${jsString({
  title: 'Optional Courses',
  courses: optionalCourses,
})};\n\nexport default IPE_SYLLABUS_OPTIONAL;\n`;
fs.writeFileSync(path.join(outDir, 'optional.js'), optionalContent, 'utf8');

const indexContent = `${termImports.join('\n')}
...
export const IPE_SYLLABUS = {
  sourceFile: 'sylla/iemcuriculmn.json',
  terms: TERM_SYLLABUS,
...
console.log(`Generated IPE syllabus files in ${outDir}`);
